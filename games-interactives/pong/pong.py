	...
	while not gameOver:
		...
		while not roundOver:
			...
			if event.type == pygame.KEYDOWN: #handles keydowns
				...
				if event.key == pygame.K_w: #playerLeft wants to go up
					playerLeftDirection = 'up'

				if event.key == pygame.K_s: #playerLeft wants to go down
					playerLeftDirection = 'down'

				if event.key == pygame.K_UP: #playerRight wants to go up
					playerRightDirection = 'up'

				if event.key == pygame.K_DOWN: #playerRight wants to go down
					playerRightDirection = 'down'

			if event.type == pygame.KEYUP: #handles keyups

				if event.key == pygame.K_w or event.key == pygame.K_s: #playerLeft wants to stop moving
					playerLeftDirection = ''

				if event.key == pygame.K_UP or event.key == pygame.K_DOWN: #playerRight wants to stop moving
					playerRightDirection = ''
